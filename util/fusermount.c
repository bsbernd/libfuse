 ...
 * Machine-readable output is a network-order uint64_t in hexadecimal.
...
static void show_features(bool text)
{
	...
	if (text) {
		...
		uint64_t network_features = htobe64(features);
		const unsigned char *feature_bytes =
			(const unsigned char *) &network_features;

		for (size_t byte_idx = 0; byte_idx < sizeof(network_features);
		     byte_idx++)
