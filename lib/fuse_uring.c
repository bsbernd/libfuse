...
static cpu_set_t *fuse_create_cpu_set(size_t nr_queues)
{
	...
	fuse_log(FUSE_LOG_DEBUG, "%s: nr_queues=%zu, nr_cpus=%d, nr_nodes=%d\n",
		 __func__,
		 nr_queues, nr_cpus, nr_nodes);
...
static struct fuse_ring_pool *fuse_create_ring(struct fuse_session *se)
{
	...
	fuse_log(FUSE_LOG_DEBUG, "nr_queues=%zu, q-depth=%d\n",
	...
	for (size_t cpu = 0; cpu < nr_procs; cpu++) {
		...
		if (numa_node < 0) {
			fuse_log(FUSE_LOG_WARNING,
				 "CPU %zu has invalid NUMA node %d, defaulting to 0\n",
				 cpu, numa_node);
			numa_node = 0;
		}
